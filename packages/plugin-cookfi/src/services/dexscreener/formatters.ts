...
export function formatDexScreenerData(data: DexScreenerAPIResponse): {
    ...
    if (!data.pairs || !Array.isArray(data.pairs)) {
        ...
        return { tickers: [], marketData: [], addresses: [] };
    }
    ...
    try {
        ...
        return {
            ...
            addresses: formattedTokens.map(token => token.address)
        };
    } catch (error) {
...
export function formatTokenData(detailedTokenData: any[]): {
    ...
    const scores = detailedTokenData.map(token => ({
        tokenName: token.name,
        tokenSymbol: token.symbol,
        trustScore: token.trustScore,
        riskLevel: token.riskLevel,
        tradingAdvice: token.tradingAdvice,
        reason: `${token.name} (${token.symbol}): ${token.reason}`
    }));

    return {
        tickers: detailedTokenData.map(token => `$${token.symbol}`),
        tokenNames: detailedTokenData.map(token => token.name),
        ...
        addresses: detailedTokenData.map(token => token.address),
        scores
    };
} 