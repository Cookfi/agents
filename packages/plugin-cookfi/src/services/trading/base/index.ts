...
export class BaseTradingService {
    ...
    async swapWithRetry(params: BaseSwapParams): Promise<BaseSwapResponse> {
        let retryCount = 0;
        let currentSlippage = params.slippage || BASE_TRADING_CONFIG.DEFAULT_SLIPPAGE;
        let lastError: Error | null = null;

        // Check initial slippage is within bounds
        if (currentSlippage > BASE_TRADING_CONFIG.MAX_SLIPPAGE) {
            const error = new Error(
                `Initial slippage ${currentSlippage.toFixed(1)}% exceeds maximum allowed ${BASE_TRADING_CONFIG.MAX_SLIPPAGE}%`
            );
        ...
        while (retryCount < BASE_TRADING_CONFIG.MAX_RETRIES) {
            try {
                elizaLogger.log(`Swap attempt ${retryCount + 1}/${BASE_TRADING_CONFIG.MAX_RETRIES}`, {
                    slippage: `${currentSlippage}%`,
                    fromToken: params.fromToken,
                    toToken: params.toToken,
                    amount: params.amount,
                    network: params.networkId || BASE_TRADING_CONFIG.DEFAULT_NETWORK
                });
                ...
                elizaLogger.warn(`Swap failed, attempt ${retryCount}/${BASE_TRADING_CONFIG.MAX_RETRIES}`, {
                    error: lastError.message,
                    currentSlippage: `${currentSlippage}%`,
                    nextSlippage: `${Math.min(currentSlippage * 2, BASE_TRADING_CONFIG.MAX_SLIPPAGE)}%`
                });
                
                const nextSlippage = Math.min(currentSlippage * 2, BASE_TRADING_CONFIG.MAX_SLIPPAGE);
                
                if (retryCount >= BASE_TRADING_CONFIG.MAX_RETRIES || nextSlippage > BASE_TRADING_CONFIG.MAX_SLIPPAGE) {
                    break;
                }

                currentSlippage = nextSlippage;
                await new Promise(resolve => setTimeout(resolve, BASE_TRADING_CONFIG.RETRY_DELAY));
            }
        }
        ...
        elizaLogger.error("All swap attempts failed:", {
            attempts: retryCount,
            maxSlippageReached: currentSlippage >= BASE_TRADING_CONFIG.MAX_SLIPPAGE,
    ...
    async swap(params: BaseSwapParams): Promise<BaseSwapResponse> {
        try {
            ...
            return {
                ...
                networkId: params.networkId || BASE_TRADING_CONFIG.DEFAULT_NETWORK
            };
