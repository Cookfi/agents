...
export class MoralisService {
    ...
    constructor() {
        if (!process.env.COOKFI_MORALIS_API_KEY) {
            throw new Error('COOKFI_MORALIS_API_KEY environment variable is required');
        }
    }

    async getExperiencedBuyerTokens(params: MoralisSearchParams = {}): Promise<TokenResult[]> {
        try {
            const searchParams = {
            ...
            elizaLogger.error('Failed to fetch experienced buyer tokens:', error);
            return [];
        }
    }
}
